...
// scripts/wallLogic.ts
function buildWall(size2, height, startHeight, buildDist, axis, block, stableCord, variableCord) {
...
function askFormat(groupSize) {
  if (players.length < groupSize || groupSize < 1) {
    world2.sendMessage("El n\xFAmero de jugadores por equipo no es correcto");
    world2.sendMessage("El m\xEDnimo de jugadores por equipo es 0 y el m\xE1ximo " + players.length);
    return false;
  }
  if (players.length / groupSize > 8) {
    world2.sendMessage("El m\xE1ximo numero de equipos es 8");
    world2.sendMessage("Aumenta el numero de miembros por equipo");
    return false;
  }
  return true;
}
...
function spreadPlayers(size2) {
  ...
  if (teams.length == 2) {
    for (let p of teams[0])
      teleportPlayer2(p, corner.x, corner.z, playersSpawnPoint);
    nextCorner();
    nextCorner();
  ...
  if (teams.length == 6) {
    for (let p of teams[4])
      teleportPlayer2(p, middle.x, middle.z, playersSpawnPoint);
    nextMiddle();
    nextMiddle();
...
function startRanDomChoosePlayers(params) {
  let miembrosEquipo = params[0];
  makeGroupsRandomChoosePlayers(miembrosEquipo);
}
function startRandomChooseTeams(params) {
  let teams3 = params[0];
  let little = params[1];
  let nLittle = params[2];
  makeGroupsRandomChooseTeams(teams3, little, nLittle);
}
function startManual(params) {
...
var sentence = "lore ipsum";
var seleccionMade = false;
var manualSelected = false;
var canceled = false;
var someoneChoosing = false;
var playerChoosing = "lore ipsum";
var concurrentAux = 0;
var playersConfirmed = /* @__PURE__ */ new Set();
var playersNotConfirmed = /* @__PURE__ */ new Set();
var playersWithoutTeam = /* @__PURE__ */ new Set();
var teams2 = [];
var playersTeam = /* @__PURE__ */ new Map();
function buildBody() {
  let body = sentence + "\n===============================\n";
...
function addProposer() {
  sentence = "\nConfiguraci\xF3n de la partida propuesta por " + playerChoosing + "\n\n";
}
function processConfirmSelection(player) {
  playersConfirmed.add(player.nameTag);
  playersNotConfirmed.delete(player.nameTag);
  world3.sendMessage(player.nameTag + " ha confirmado los ajustes");
  if (playersNotConfirmed.size == 0 && !canceled) {
    if (manualSelected) {
      controller("manualChooseTeams", [teams2]);
    }
...
function checkConcurrence(aux, player) {
  if (aux != concurrentAux) {
    player.runCommand("/title @s actionbar Alguien cancel\xF3 y no se ha procesado tu selecci\xF3n");
    return true;
  } else {
    return false;
  }
...
function buildbodyManual() {
  ...
  for (let i = 0; i < teams2.length; i++) {
    ...
    for (let p of teams2[i]) {
      sentence += "- " + p.nameTag + "\n";
    }
    if (i < teams2.length - 1)
      sentence += "\n";
  }
  if (playersWithoutTeam.size > 0) {
...
function selectTeam(player) {
  ...
  window.show(player).then((response) => {
    if (response.canceled) {
      ...
      if (playersTeam.has(player.nameTag)) {
        ...
        playersConfirmed.clear();
        world3.sendMessage("Alguien ha cambiado de equipo, vuelve a confirmar la configuraci\xF3n");
  ...
  function processTeamSelection(response) {
    teams2[response.selection].add(player);
  ...
  window.show(player).then((response) => {
    if (response.canceled) {
      ...
      for (let i = 0; i < response.selection + 1; i++) {
        teams2.push(/* @__PURE__ */ new Set());
      }
      playersWithoutTeam.clear();
      world3.getAllPlayers().forEach((player2) => {
        playersWithoutTeam.add(player2.nameTag);
      });
      playersTeam.clear();
      manualSelected = true;
...
function buildBodyChooseTeams(numberOfPlayers, numberOfTeams) {
  let little = Math.floor(numberOfPlayers / numberOfTeams);
  let nLittle = 0;
  sentence += "Se ";
  if (numberOfPlayers % numberOfTeams == 0) {
    nLittle = numberOfTeams;
    if (numberOfTeams == 1) {
      sentence += "generar\xE1:\n1 equipo de ";
    } else {
      sentence += "generar\xE1n:\n" + numberOfTeams + " equipos de ";
    }
    if (little == 1) {
      sentence += "1 miembro\n";
    } else {
...
function randomChooseTeams(player, numPlayers) {
  let randomParams = new ActionFormData().title("Random").body("\nN\xFAmero de equipos\n\n");
  let i = 0;
  ...
  randomParams.show(player).then((response) => {
    if (response.canceled) {
      ...
      let aux = -1;
      if (Number.isInteger(response.selection) && response.selection !== void 0) {
...
function randomChoosePlayers(player, numPlayers) {
  let randomParams = new ActionFormData().title("Random").body("\nN\xFAmero de jugadores por equipo\n\n");
  let first = -1;
...
function randomSelectedWindow(player, numberOfPlayers) {
  const form = new ActionFormData().title("Random").button("1. N\xFAmero de jugadores").button("2. N\xFAmero de equipos").body("\nElige una de estas dos opciones:\n\n1. N\xFAmero de jugadores:\nX n\xFAmero de equipos con los integrantes por equipo que indiques y un \xFAltimo con el resto\n\n2. N\xFAmero de equipos:\nN\xFAmero de equipos que indiques con 1 de diferencia de tama\xF1o\n\n");
...
function selectionWindow(player, numberOfPlayers) {
  const manualOrRandom = new ActionFormData().title("Equipos").body("\nProp\xF3n una forma de generar los equipos:\n\n").button("Manual").button("Random");
...
function startWindow(player) {
  if (seleccionMade) {
    confirmationWindow(player);
  } else if (!someoneChoosing) {
    let numberOfPlayers = world3.getAllPlayers().length;
    controller("setPlayers", []);
    selectionWindow(player, numberOfPlayers);
    someoneChoosing = true;
    playerChoosing = player.nameTag;
  } else {
    const aviso = new ActionFormData().title("Aviso").body("\n" + playerChoosing + " est\xE1 eligiendo una forma de generar los equipos, espera a que termine");
    aviso.show(player);
  }
}

// scripts/mainMenuWindows.ts
import { ActionFormData as ActionFormData2, ModalFormData as ModalFormData2 } from "@minecraft/server-ui";
import { world as world4 } from "@minecraft/server";
var phase = 1;
var seleccionMade2 = false;
var someoneChoosing2 = false;
var playerChoosing2;
var lifes;
var time;
var spawnLocation;
...
function gameSettingsWindow(player) {
  if (seleccionMade2) {
    ...
    if (!someoneChoosing2) {
      someoneChoosing2 = true;
      playerChoosing2 = player.nameTag;
      const configurationWindow = new ModalFormData2().title("Configuraci\xF3n de la partida").textField("\nDuraci\xF3n de la partida (en minutos)", "60", "60").dropdown("\nN\xFAmero de vidas", ["1", "2", "3", "4", "5"], 2).dropdown("Lugar spawn despu\xE9s de vida extra", ["Lugar de muerte", "Lugar de spawn"], 0);
      ...
      const aviso = new ActionFormData2().title("Aviso").body("\n" + playerChoosing2 + " est\xE1 eligiendo una configuraci\xF3n de la partida, espera a que termine");
      aviso.show(player);
    }
  }
}
function processConfirmSelection2(player) {
  ...
  world4.sendMessage(player.nameTag + " ha confirmado los ajustes");
...
function mainMenu(player) {
  if (phase == 1) {
    if (firstTutorialSeen.has(player.nameTag)) {
      ...
      const mainMenu2 = new ActionFormData2().title("UHC").body("\nBienvenido a UHC:\n\nAntes de empezar la partida es necesario que se elija en conjunto la configuraci\xF3n de la partida y el formato de los equipos.\n\nPrimero alguien debe proponer una configuraci\xF3n y despu\xE9s el resto de jugadores debe aceptar o rechazar la propuesta.\n\n").button("comenzar con la selecci\xF3n de equipos");
      mainMenu2.show(player).then((response) => {
        if (response.selection == 0) {
          windowController("startWindow", [player]);
        }
      });
    ...
    if (secondTutorialSeen.has(player.nameTag)) {
      ...
      const mainMenu2 = new ActionFormData2().title("UHC").body(
        "\nYa formados los equipos ahora es necesario configurar la duraci\xF3n de la partida y el n\xFAmero de vidas.\n\n"
      ).button("comenzar con la configuraci\xF3n de la partida");
      mainMenu2.show(player).then((response) => {
        if (response.selection == 0) {
          gameSettingsWindow(player);
        }
...
function processPlayerDie(event) {
  ...
  if (event.damageSource.damagingEntity?.typeId == "minecraft:player" || !checkLives(event.deadEntity)) {
    ...
    for (let i = 0; i < 36; i++) {
      const item = inventory.container?.getSlot(i).getItem();
      if (item == void 0)
        continue;
      world5.getDimension("overworld").spawnItem(item, location);
    }
...
function teleportPlayer3(player) {
  ...
  player.addEffect(MinecraftEffectTypes.InstantHealth, 160, { amplifier: 255, showParticles: false });
  player.addEffect(MinecraftEffectTypes.Saturation, 160, { amplifier: 255, showParticles: false });
  player.teleport({ x, y: 320, z }, { dimension: world5.getDimension("overworld") });
}
function blockPvp(player) {
  if (deathMessage == void 0)
    deathMessage = /* @__PURE__ */ new Map();
  if (deathMessage.get(player.nameTag) == void 0)
    deathMessage.set(player.nameTag, 0);
  deathMessage.set(player.nameTag, deathMessage.get(player.nameTag) + 1);
  ...
  subscription = world5.afterEvents.entityHurt.subscribe((event) => {
    ...
    if (damageSource?.damagingEntity?.nameTag == player.nameTag && damageSource?.damagingEntity?.typeId === "minecraft:player" && hurtEntity?.typeId === "minecraft:player") {
...
function windowController(event, params) {
  switch (event) {
    case "startWindow":
  ...
  return new Promise((resolve) => {
    ...
    let id = system4.runInterval(() => {
      if (tiempoRestante <= 0) {
        ...
        world7.getDimension("overworld").runCommand("/execute as @a at @s run playsound note.bell @s ~ ~ ~ ");
        world7.getDimension("overworld").runCommand("/title @a title " + tiempoRestante);
        tiempoRestante--;
      }
    }, 20);
  });
