...
let middle: {x:number; z:number} = {x: 0, z: 0};
let players:any[];
export let teams:any[][] = [];

//selecciona la esquina desde la que se va a empezar
function firstCorner(size:number){
...
function firstMiddle(size:number){
  middle.x = 0;
  middle.z = 0;
  let aux:number;
  let random:number = Math.random() * 10;
...
function makeGroupsManual(teamsAux:Set<Player>[]) {
...
function shuffleArray(groups:any[]){
...
function askFormat(groupSize: number): boolean {
...
function makeGroupsRandomChooseTeams(nTeams:number, little:number, nLittle:number){
...
function makeGroupsRandomChoosePlayers(groupSize:number){
...
export function startRanDomChoosePlayers(params:any[]) {
  let miembrosEquipo = params[0];
  makeGroupsRandomChoosePlayers(miembrosEquipo);
}

export function startRandomChooseTeams(params:any[]) {
  let teams = params[0];
  let little = params[1];
...
export function startManual(params:any[]){
